...
疫苗和疾病从“医疗”Tab 手动添加。新增或调整药物时，在“记录”Tab 选择吸入药或口服药，直接填写新的药名、剂量和频率；数据库会与同类型的上一条记录比较，只要药名、剂量、剂量单位或频率发生变化，就会自动在医疗时间轴写入一条“用药变化”。首次运行医疗时间轴脚本时，也会补齐已有记录中的历史变化。
...
`supabase/add_medical_history.sql`。该脚本可重复运行，会创建或升级医疗记录表、补齐历史用药变化，并启用后续自动记录；同时沿用现有两位照护者的共享访问权限。
